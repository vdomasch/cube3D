...
/*   Updated: 2024/09/03 10:53:39 by vdomasch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*int	get_pixel_color(t_data *data, t_image *image, char *path, int x, int y)
{
	...
	printf("image->img = %p, image->addr = %s, image->bits_per_pixel = %d, image->line_length = %d, image->endian = %d\n", image->img, image->addr, image->bits_per_pixel, image->line_length, image->endian);
...
/*int	get_pixel_texture_color(t_data *data, t_raycast *raycast, int y)
{
	...
	if (side == 0)
	{
		return (get_pixel_color(data, &data->texture.east, data->texture.e_path, raycast->wall_x, y));
		printf("		get_pixel_texture_color end 1\n");
	}
	else if (side == 1)
	{
		return (get_pixel_color(data, &data->texture.west, data->texture.w_path, raycast->wall_x, y));
		printf("		get_pixel_texture_color end 2\n");
	}
	else if (side == 2)
	{
		return (get_pixel_color(data, &data->texture.south, data->texture.s_path, raycast->wall_x, y));
		printf("		get_pixel_texture_color end 3\n");
	}
	else
	{
		return (get_pixel_color(data, &data->texture.north, data->texture.n_path, raycast->wall_x, y));
...
void	draw_line(t_data *data, t_raycast *raycast, int x, t_image *image)
{
	...
	while (y < data->res.y)
	{
		if (y < raycast->draw_start)
			ft_pixel_put(image, x, y, data->texture.ceiling_color);
		else if (y >= raycast->draw_start && y <= raycast->draw_end)
			y += raycast->draw_end - raycast->draw_start;
			//ft_pixel_put(data, x, y, get_pixel_texture_color(data, raycast, y));
		else
			ft_pixel_put(image, x, y, data->texture.floor_color);
		y++;
	}
